...
type BulanRanker interface {
	Create(*abstraction.Context, *model.BulanRanker) (*model.BulanRanker, error)
	Upsert(*abstraction.Context, *model.BulanRanker) (*model.BulanRanker, error)
	...
	Find(*abstraction.Context, *model.BulanRankerFilter, *abstraction.Pagination) ([]dto.BulanRankerGetResponse, *abstraction.PaginationInfo, error)
...
func NewBulanRanker(db *gorm.DB) *bulanRanker {
	return &bulanRanker{
		abstraction.Repository{
			Db: db,
		},
...
func (r *bulanRanker) Create(ctx *abstraction.Context, m *model.BulanRanker) (*model.BulanRanker, error) {
	conn := r.CheckTrx(ctx)
...
func (r *bulanRanker) Upsert(ctx *abstraction.Context, m *model.BulanRanker) (*model.BulanRanker, error) {
	conn := r.CheckTrx(ctx)
...
func (r *bulanRanker) Find(ctx *abstraction.Context,
	m *model.BulanRankerFilter, p *abstraction.Pagination) ([]dto.BulanRankerGetResponse,
	*abstraction.PaginationInfo, error) {
	conn := r.CheckTrx(ctx)

	var err error
	var count int64
	var result []dto.BulanRankerGetResponse
	var info abstraction.PaginationInfo

	//partQuery := fmt.Sprintf("tas.thn_ang_id = %d and tas.satker_id = %d and wpr.deleted_at is NULL",
	//*m.ThnAngID, *m.SatkerID)

	query := conn.Table("bulan_ranker br").
		Select(
			`br.id, br.name
	`)

	query = r.Filter(ctx, query, *m).Where("br.deleted_at is NULL")
	queryCount := query
...
func (r *bulanRanker) checkTrx(ctx *abstraction.Context) *gorm.DB {
	if ctx.Trx != nil {
		return ctx.Trx.Db
	}
	return r.Db
