...
type Bulan interface {
	...
	Find(*abstraction.Context, *model.BulanFilter, *abstraction.Pagination) ([]model.Bulan, *abstraction.PaginationInfo, error)
...
func NewBulan(db *gorm.DB) *bulan {
	return &bulan{
		abstraction.Repository{
			Db: db,
		},
...
func (r *bulan) Find(ctx *abstraction.Context, m *model.BulanFilter, p *abstraction.Pagination) ([]model.Bulan, *abstraction.PaginationInfo, error) {
	conn := r.CheckTrx(ctx)

	var err error
	var count int64
	var result []model.Bulan
...
func (r *bulan) checkTrx(ctx *abstraction.Context) *gorm.DB {
	if ctx.Trx != nil {
		return ctx.Trx.Db
	}
	return r.Db
