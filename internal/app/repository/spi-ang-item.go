...
type SpiAngItem interface {
	Create(*abstraction.Context, *model.SpiAngItem) (*model.SpiAngItem, error)
	...
	Find(*abstraction.Context, *model.SpiAngItemFilter, *abstraction.Pagination) (*[]model.SpiAngItem, *abstraction.PaginationInfo, error)
...
func NewSpiAngItem(db *gorm.DB) *spiAngItem {
	return &spiAngItem{
		abstraction.Repository{
			Db: db,
		},
...
func (r *spiAngItem) Create(ctx *abstraction.Context, m *model.SpiAngItem) (*model.SpiAngItem, error) {
	conn := r.CheckTrx(ctx)

	err := conn.FirstOrCreate(&m, map[string]interface{}{"spi_ang_id": m.SpiAngID, "komponen_id": m.KomponenID}).
...
func (r *spiAngItem) Update(ctx *abstraction.Context, m *model.SpiAngItem) (*model.SpiAngItem, error) {
	conn := r.CheckTrx(ctx)
...
func (r *spiAngItem) Delete(ctx *abstraction.Context, m *model.SpiAngItem) (*model.SpiAngItem, error) {
	conn := r.CheckTrx(ctx)
...
func (r *spiAngItem) FindByID(ctx *abstraction.Context, m *model.SpiAngItem) (*model.SpiAngItem, error) {
	conn := r.CheckTrx(ctx)
...
func (r *spiAngItem) Find(ctx *abstraction.Context, m *model.SpiAngItemFilter, p *abstraction.Pagination) (*[]model.SpiAngItem, *abstraction.PaginationInfo, error) {
	conn := r.CheckTrx(ctx)

	var err error
	var count int64
	var result []model.SpiAngItem
	var info abstraction.PaginationInfo

	query := conn.Model(&model.SpiAngItem{})
...
/*
func (r *spiAngItem) ViewSpiSdmItemBySpiSdmID(ctx *abstraction.Context, m *dto.SpiSdmItemViewBySpiSdmIDRequest) ([]dto.SpiSdmItemViewBySpiSdmIDResponse, error) {
	conn := r.CheckTrx(ctx)
...
*/
func (r *spiAngItem) checkTrx(ctx *abstraction.Context) *gorm.DB {
	if ctx.Trx != nil {
		return ctx.Trx.Db
	}
	return r.Db
