...
type handler struct {
	service wbk.WbkSubProgramRankerService
}

var err error

func NewHandler(f *factory.Factory) *handler {
	service := wbk.NewWbkSubProgramRankerService(f)
...
func (h *handler) Save(c echo.Context) error {
	...
	payload := new(wbk2.WbkSubProgramRankerUpsertRequest)
...
func (h *handler) Get(c echo.Context) error {
	...
	payload := new(wbk2.WbkSubProgramRankerGetRequest)
	if err := c.Bind(payload); err != nil {
		//fmt.Println(1)
		return res.ErrorBuilder(&res.ErrorConstant.BadRequest, err).Send(c)
	}
