...
type handler struct {
	service wbk.WbkProgramRankerService
}

var err error

func NewHandler(f *factory.Factory) *handler {
	service := wbk.NewWbkProgramRankerService(f)
...
func (h *handler) Save(c echo.Context) error {
	...
	payload := new(wbk2.WbkProgramRankerUpsertRequest)
...
func (h *handler) GetSatkerNilaiByThnAngID(c echo.Context) error {
	...
	payload := new(wbk2.WbkProgramRankerGetRequest)
	if err := c.Bind(payload); err != nil {
		//fmt.Println(1)
		return res.ErrorBuilder(&res.ErrorConstant.BadRequest, err).Send(c)
	}
	...
	result, err := h.service.GetSatkerNilaiByThnAngID(cc, payload)
	if err != nil {
		return res.ErrorResponse(err).Send(c)
	}
