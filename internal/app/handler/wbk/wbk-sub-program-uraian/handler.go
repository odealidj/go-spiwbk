...
type handler struct {
	service wbk.WbkSubProgramUraianService
}

var err error

func NewHandler(f *factory.Factory) *handler {
	service := wbk.NewWbkSubProgramUraianService(f)
...
func (h *handler) Save(c echo.Context) error {
	...
	payload := new(wbk2.WbkSubProgramUraianUpsertRequest)
...
func (h *handler) Get(c echo.Context) error {
	...
	payload := new(wbk2.WbkSubProgramUraianGetRequest)
	if err := c.Bind(payload); err != nil {
		//fmt.Println(1)
		return res.ErrorBuilder(&res.ErrorConstant.BadRequest, err).Send(c)
	}
