...
type handler struct {
	service spi_pbj.JenisRekapitulasiService
}

var err error

func NewHandler(f *factory.Factory) *handler {
	service := spi_pbj.NewJenisRekapitulasiService(f)
...
func (h *handler) Get(c echo.Context) error {
	...
	payload := new(spi_pbj2.JenisRekapitulasiGetRequest)
