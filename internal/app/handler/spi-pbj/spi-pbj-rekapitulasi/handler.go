...
type handler struct {
	service spi_pbj.SpiPbjRekapitulasiService
}

func NewHandler(f *factory.Factory) *handler {
	service := spi_pbj.NewSpiPbjRekapitulasiService(f)
...
func (h *handler) Save(c echo.Context) error {
	...
	payload := new(spi_pbj2.SpiPbjRekapitulasiSaveRequest)
...
func (h *handler) Upsert(c echo.Context) error {
	...
	payload := new(spi_pbj2.SpiPbjRekapitulasiUpsertRequest)
	if err = c.Bind(payload); err != nil {
		return res.ErrorBuilder(&res.ErrorConstant.BadRequest, err).Send(c)
	}
	if err = c.Validate(payload); err != nil {
...
func (h *handler) GetSpiPbjRekapitulasiByID(c echo.Context) error {
	...
	payload := new(spi_pbj2.SpiPbjRekapitulasiGetRequest)
	if err := c.Bind(payload); err != nil {
		//fmt.Println(1)
		return res.ErrorBuilder(&res.ErrorConstant.BadRequest, err).Send(c)
	}
